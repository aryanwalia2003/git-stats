...
import (
	"fmt" // fmt is for formatting strings like Sprintf

	"github.com/aryanwalia2003/git-stats/internal/domain" // our domain entities
)
...
func (r *Reader) GetRecentCommits(limit int) ([]domain.Stat, error) {
	...
	maxCount := fmt.Sprintf("--max-count=%d", limit) // builds "--max-count=10"
	...
	return parseCommitLog(output), nil // delegate parsing to parse_commits.go
}
